...
2. **锁内事务式 apply**：apply 阶段在 `withPrivateFileLock(statePath, …)` 内执行：
   ...
   - **fresh-config 围栏**：overlay provision / state 写之前，重新读 xacpx `config.json`，对每条 validated update 复检（agent 仍存在、driver 未变、effective default argv 仍等于 planned argv、provenance 仍允许 Path A）。并发写入的显式 `agents.<name>.argv` 不能被「把 stale default 提升成 sticky step 2」永久遮蔽。
