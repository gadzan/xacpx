...
| `xacpx migrate argv [--dry-run]` | Backfill structured `argv` into `state.json` sessions whose recorded raw command contains whitespace (the Windows fail-closed trigger) and into the corresponding `agents.<name>.argv`. Runs automatically at every daemon startup; this command is for explicit invocation and troubleshooting. Exit `0` when nothing was skipped, `1` otherwise. See [config-reference.md](./config-reference.md#windows-raw-command-migration) for context |
