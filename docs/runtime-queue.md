...
- **Drain:** `RuntimeEngine.drainLoop` is single-flight per `logicalSessionId` (`draining` map), reuses `executeRuntimeTurn` (same lifecycle as direct prompt), dequeues only on terminal `completed`/`failed`/`cancelled`/`permission_denied` (crash keeps head for at-least-once replay), `primeQueuesFromCatalog` enumerates journals at bridge startup for authoritative runtime-bound sessions.
