...
| `/agent add <codex|claude|pi|openclaw|gemini|cursor|copilot|droid|factory-droid|factorydroid|grok-build|hermes|iflow|kilocode|kimi|kiro|mux|omp|opencode|qoder|qwen|reasonix|trae>` | Add a built-in agent template; an existing agent with the same name but a different configuration will not be overwritten |
