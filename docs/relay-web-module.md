...
- ghostty-web 点画布会聚焦其 1×1 clipped textarea，而 keydown/composition 监听在宿主上。看板不再把该 textarea 铺满宿主（那会让 Windows IME 候选框锚到整个宿主的错误几何位置）：adapter 的 `syncGhosttyInputAnchor` 把它按 canvas cell 几何锚到 shell 光标所在单元格（不可见、`pointer-events:none`），`focus()` 直接聚焦该 textarea（ghostty 0.4 的 `Terminal.focus()` 聚焦的是宿主元素）；open/write/resize/resetAndReplay/focus 后都会重新锚定。IME composition（`compositionstart/update/end`、`isComposing`、keyCode 229）全部由 ghostty-web 0.4 自己处理，看板不重复实现，最终只提交 committed 文本。底部快捷键栏的 Enter 不依赖画布焦点，可作对照。
