...
- 终端渲染用 `@xterm/xterm`（DOM 渲染器，随终端 Tab 按需 lazy chunk 加载，`xterm.css` 一并懒加载，无 WASM 资产）。xterm 自己把不可见的 helper textarea 锚定在光标所在单元格（随光标移动更新），IME composition（`compositionstart/update/end`、`isComposing`、keyCode 229）由 xterm 内建 CompositionHelper 处理、最终只提交 committed 文本——adapter 不做任何 IME 锚定 hack，`focus()` 直接 `Terminal.focus()`。adapter 的 fit()/localGeometry() 用 `.xterm-screen` 渲染度量（xterm 把它设为精确 cols×cellW × rows×cellH）。iOS 中文 IME 的确认/空格/中文标点常走 `input` `insertText`（`composed: true`、keydown 已置 `_keyDownSeen`），stock `@xterm/xterm@6.0.0` 会丢掉这些事件；adapter 在构造后对 `_inputEvent` 做 xterm.js#5614 的本地回移（未在 composing / pending compositionend send 时放行），不把 helper textarea 拉成全屏。真实 iOS Safari 仍需实机验证。
