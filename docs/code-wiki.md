...
On the xacpx side of the same boundary, a `LogicalSession` whose recorded `transport_agent_command` contains whitespace used to be unresolvable on Windows because acpx rejects raw `--agent` strings there. The daemon-side counterpart to `migrateSessionArgvFile` is [`migrateStateAgentArgv`](../src/state/auto-migrate-agent-argv.ts) — pure decision `evaluateStateSessionArgvMigration` plus the I/O seam. It runs once at every daemon startup between `StateStore.load()` and `SessionService` construction; the apply phase is fail-closed all-or-nothing with three gates: per-agent disagreement on argv identity (covering multi-token, single-token, AND unknown-identity sessions via the shared `isDerivedAgentArgv` classification), fresh-state mismatch, or config patch conflict each abort the whole transaction. The fail-closed guarantee in `resolveLaunchSpec` ([src-sessions/session-service.ts](../src/sessions/session-service.ts)) stays unchanged for sessions where identity cannot be proven.
