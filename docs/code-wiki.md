...
- State and sessions:
  ...
  - `migrateStateAgentArgv()` is invoked by the console lifecycle AFTER the runtime consumer lock is acquired (not inside `buildApp`, which loads shared config/state before the lock — running the migration there would race a concurrent `xacpx migrate argv` and boot with a stale in-memory copy); it always reloads config+state in place afterward. For any `state.json` session whose recorded `transport_agent_command` is a multi-token raw command, or a single-token command that does NOT round-trip losslessly as `[command]` (the Windows fail-closed triggers), it backfills `transport_agent_argv` from either the matching `agents.<name>.argv` or the on-disk acpx session record (`~/.acpx/sessions/index.json`). The apply phase is a single locked transaction with three fail-closed gates: (1) planning-time per-agent safety bucket covering ALL non-sticky sessions for the agent, using the SHARED `isDerivedAgentArgv` classification (exported from [src/config/agent-launch.ts](../src/config/agent-launch.ts)) so the sticky check exactly matches `resolveLaunchSpec` step 2; sessions with no recorded identity are also treated as unsafe; (2) fresh-state per-agent recheck under lock catching new sessions added between planning and lock acquire; (3) fresh-state per-session recheck under lock. Config patch and state write happen only after all three gates pass; any I/O failure or conflict aborts the whole transaction. A planned argv reaches the session as a **session-local structured launch** via a `xacpx-managed-<driver>-<hash>` alias persisted on the session itself (`transport_acpx_agent` + `transport_agent_argv`) — NEVER as a global `agents.<name>.argv`. The migration is purely session-local so future bare sessions of the same driver continue to resolve through acpx and honor `~/.acpx/config.json` global and `<cwd>/.acpxrc.json` project overrides. The session-local alias is provisioned only when the default's provenance is NOT `derived` (managed adapter npx pin, hermes shim, opencode / kilocode local fallback) — derived launches are recomputed on restart from the current pin / config and must not be frozen. The apply phase provisions the new session overlays into `~/.acpx/config.json` BEFORE writing state (so acpx can resolve `--agent <alias>` to the exact argv), aborts the whole transaction if provisioning throws, and never writes the xacpx `config.json`. On every startup `autoMigrateArgv` re-provisions session overlays from the live state (idempotent, content-hashed) so aliases survive `~/.acpx/config.json` cleanup. Historical custom argv that does not match the current default is skipped entirely. `result.configUpdates` is committed only after `patchConfig` resolves, so a patch failure does not leave a stale "updated" report. Idempotent; logs each migration at `info` and each skip/error at `warn`. CLI surface: `xacpx migrate argv [--dry-run]` — the mutating path acquires and holds the same runtime consumer lock every console instance holds, so it refuses (non-zero) while a daemon or foreground runtime is alive. Module: [src/state/auto-migrate-agent-argv.ts](../src/state/auto-migrate-agent-argv.ts)
- transport selection:
...
Structured launches (managed pins, hermes shim, local fallbacks, user `argv`) are modeled as an `AgentLaunchSpec` ([agent-launch.ts](../src/config/agent-launch.ts)): `acpxAgent` (positional: content-addressed alias or bare driver), `agentCommand` (canonical acpx session identity), `agentArgv`, and `rawCommand` (Unix-only legacy override). Alias provisioning merges `{argv}` entries into `~/.acpx/config.json` under a proper-lockfile ([acpx-agent-overlay.ts](../src/transport/acpx-agent-overlay.ts)); legacy session records are backfilled with `agent_argv` only on exact identity match ([acpx-session-argv-migration.ts](../src/transport/acpx-session-argv-migration.ts)). Persisted preinstalled commands count as derived managed identity only when they match the trusted runtime adapter root and the complete immutable release/package layout; the broader path-shape classifier is fencing-only. The agent selection rule lives in exactly one place — `appendAgentAndTail` in [acpx-command-builder.ts](../src/transport/acpx-command-builder.ts) — and every transport and management by-pass (status/list/reap/doctor) routes through it.

On the xacpx side of the same boundary, a `LogicalSession` whose recorded `transport_agent_command` is a multi-token raw command, or a single-token command that does NOT round-trip losslessly as `[command]`, used to be unresolvable on Windows because acpx rejects raw `--agent` strings there. The daemon-side counterpart to `migrateSessionArgvFile` is [`migrateStateAgentArgv`](../src/state/auto-migrate-agent-argv.ts) — pure decision `evaluateStateSessionArgvMigration` plus the I/O seam. It is invoked by the console lifecycle AFTER the runtime consumer lock (never inline in `buildApp`, which loads shared state before the lock) and reloads state in place; the apply phase is fail-closed all-or-nothing with the gates above (per-agent disagreement on argv identity covering multi-token, lossy single-token, AND unknown-identity sessions via the shared `isDerivedAgentArgv` classification; per-session fresh-state validation; overlay provisioning) and writes the session with `transport_acpx_agent` + `transport_agent_argv` (NO global `config.json` write). A historical custom argv that does not match the current driver's default is skipped entirely, and `derived` launches (managed pin / hermes / local fallback) are never pinned to a session-local alias — the session stays fail-closed on its raw command until migrated manually. Future bare sessions of any driver continue to honor acpx global / `.acpxrc.json` project overrides because the migration never writes a global argv that would shadow them, and the fail-closed guarantee in `resolveLaunchSpec` ([src-sessions/session-service.ts](../src/sessions/session-service.ts)) stays unchanged for sessions where the identity cannot be proven or the default is derived.
