...
- State and sessions:
  ...
  - `migrateStateAgentArgv()` is invoked by the console lifecycle AFTER the runtime consumer lock is acquired (not inside `buildApp`, which loads shared config/state before the lock — running the migration there would race a concurrent `xacpx migrate argv` and boot with a stale in-memory copy); it always reloads config+state in place afterward. For any `state.json` session whose recorded `transport_agent_command` is a multi-token raw command, or a single-token command that does NOT round-trip losslessly as `[command]` (the Windows fail-closed triggers), it backfills structured argv as a **session-local** `xacpx-managed-<driver>-<hash>` alias (`transport_acpx_agent` + `transport_agent_argv`) from either matching `agents.<name>.argv` or the on-disk acpx session record — NEVER as global `agents.<name>.argv`. Path A plans per-session (siblings do not gate each other). Fully migrated = argv + canonical alias; argv-only is a repair. Derived defaults are skipped; historical argv ≠ current default is skipped. Apply: lock state → fresh per-session validate (invalid sessions skip individually) → `ensureAgentOverlays` before state write (provision failure aborts, no state write) → write validated sessions. On every startup `autoMigrateArgv` re-provisions overlays via `computeSessionOverlayEntries` with an ownership gate (only `xacpx-managed-*` aliases equal to `deriveAgentAlias(driver, argv)`; fail-closed). Idempotent; logs each migration at `info` and each skip/error at `warn`. CLI: `xacpx migrate argv [--dry-run]`. Module: [src/state/auto-migrate-agent-argv.ts](../src/state/auto-migrate-agent-argv.ts)
- transport selection:
