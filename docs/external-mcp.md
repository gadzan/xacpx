...
`auto` and `queue` both use the target acpx session's next-turn queue; `auto` never interrupts. As of Peer Interrupt Delivery v0.4, explicit `mode: "interrupt"` for managed logical sessions is an xacpx-managed **cancellation-request + true-settlement + priority-next-turn** primitive: the target's `TurnQueue` reserves one interrupt slot, requests cancellation of the current turn through its own control plane (AbortController signal + transport cancel, exactly once), waits for the turn to truly settle, then runs the interrupting message ahead of the ordinary FIFO queue. xacpx guarantees lane ordering and no overlap; whether the underlying runtime actually terminates the active model turn early is transport-owned — the predecessor may terminalize as `cancelled` or `completed`, and xacpx never rewrites a natural completion into a cancellation. No provider-native steering support is required. Only endpoints advertising `interrupt: true` (managed logical sessions) accept it; workers, external coordinators, and stale-capability destinations fail closed with `TARGET_NOT_INTERRUPTIBLE` before any cancellation. A second distinct interrupt while one is pending is rejected `MESSAGE_QUEUE_FULL`; duplicate retries of the same message are deduplicated. `steer` still requires native provider live-input support and never silently degrades. Offline store-and-forward is not part of `agent_send`.
