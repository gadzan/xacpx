...
`auto` and `queue` both use the target acpx session's next-turn queue; `auto` never interrupts. As of Peer Interrupt Delivery v0.4, explicit `mode: "interrupt"` for managed logical sessions is implemented by xacpx itself: the target's `TurnQueue` reserves one interrupt slot, cancels the current turn through its own control plane, waits for the turn to truly settle, then runs the interrupting message ahead of the ordinary FIFO queue — no provider-native steering support is required. Only endpoints advertising `interrupt: true` (managed logical sessions) accept it; workers, external coordinators, and stale-capability destinations fail closed with `TARGET_NOT_INTERRUPTIBLE` before any cancellation. A second distinct interrupt while one is pending is rejected `MESSAGE_QUEUE_FULL`; duplicate retries of the same message are deduplicated. `steer` still requires native provider live-input support and never silently degrades. Offline store-and-forward is not part of `agent_send`.
