...
- `instance.state.sync` handler (in the same instance-gateway dispatch as `instanceEvent`):
  ...
  - for each `finishedOffline` entry, append the out row (`text`, or `errorMessage` for `ok:false`), mirroring existing flush semantics; **skip** when a turn with the same alias is already present in the sync's `turns` (shouldn't happen, defends against weird ordering). **Dedup is two-layered**, because the connector legitimately re-sends after an unconfirmed flush and blind appends would duplicate rows:
    1. an in-memory fingerprint set (`instanceId, alias, prompt, outText`, bounded FIFO at 128) of entries THIS hub process already persisted — exact idempotence for the common redelivery (unconfirmed flush, extra reconnect) where the content match cannot distinguish a redelivery from a genuinely repeated reply;
    2. a SQLite recency fallback (`messages.listBySession(accountId, instanceId, alias, { limit: 5 })`) for redeliveries that cross ANOTHER hub restart (the fingerprint set died with the process). When the entry carries a `prompt`, the fallback matches the adjacent `in`(prompt) → `out`(reply) **pair** a previous sync would have written — so two different turns that happen to produce identical reply text ("ok", `/status` output) are both kept; only entries WITHOUT a prompt fall back to a bare direction+text match (documented residual, see Further Notes);
...
- **Dedup residual limitations**: the in-memory fingerprint layer makes redelivery to the SAME hub process exactly idempotent; the SQLite layer covers redelivery across ANOTHER restart by pair-matching prompt+reply against the last 5 rows. Residual holes: (a) a prompt-less entry falls back to a bare text match, so a genuinely different prompt-less turn producing identical text within the last 5 rows after a double-restart could be skipped; (b) two turns with identical prompt AND identical reply within the recency window after a double-restart are indistinguishable to content-based dedup. Both require pathological content coincidences across a specific restart sequence; a turnId would eliminate them but needs a schema column, which this PR deliberately avoids.
