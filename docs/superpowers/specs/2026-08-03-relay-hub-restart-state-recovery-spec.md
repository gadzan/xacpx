...
- `instance.state.sync` handler (in the same instance-gateway dispatch as `instanceEvent`):
  ...
  - **prompt backfill (`backfillInboundPrompt`, shared by restored `turns` and `finishedOffline`)**: with a `queueItemId`, query `MessageStore.queuedState()` (three states: `pending` → `promoteQueued()` moves the persisted queued row to its execution position; `executed` → a previous `promoteQueued` retained the association in `messages.origin_queue_item_id`, so the prompt is already in history and must NOT be re-inserted — text matching cannot tell a redelivery from a user sending the identical prompt twice; `absent` → `appendQueuedFallback()`). Without a `queueItemId`, append the plain `in` row only when it is not already the trailing row — a turn that started during the outage recovers as an in+out pair instead of an orphan answer. `finishedOffline` / `turns` carry `queueItemId` / `scheduled` for this reconciliation;
