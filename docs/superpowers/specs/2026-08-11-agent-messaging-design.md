...
Implementation status (2026-08-18): Local queue-first v0.1 and Relay route
v0.1 are implemented in the current xacpx worktree: stable node/endpoint
identities, same-coordinator and multi-node Relay discovery, one-way
`agent_send`, Relay Hub message routing & directory cache, typed receipts/errors,
guardrails, both transports, daemon RPC, and MCP tools. Same-turn steering is
not advertised; it remains gated by the separate acpx/codex-acp feasibility
spike and its required GO report.
