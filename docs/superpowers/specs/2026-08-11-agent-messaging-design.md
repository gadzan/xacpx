...
- 调用方可显式选择 interrupt + new prompt；默认绝不自动 interrupt。（v0.4 起实现说明：受管逻辑会话的显式 interrupt 由 xacpx TurnQueue 以「取消请求 + 真实 settle 屏障 + 优先级下一 turn + 严格不重叠」实现，不依赖 acpx/provider 的 live-input 能力；底层 transport 是否真的提前终止当前 turn 属 transport 自有行为——predecessor 终态可为 cancelled 或 completed，xacpx 不改写自然完成。原生 steer 仍是独立可选项。见 `2026-08-24-xacpx-agent-messaging-peer-interrupt-delivery-v0.4-spec.md`。）
