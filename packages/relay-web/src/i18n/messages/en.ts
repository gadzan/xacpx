// English catalog. Each migration task appends its own namespace below.
// Values here MUST match the original hardcoded English exactly so existing
// test assertions on visible text keep passing.
export default {
  common: {
    cancel: "Cancel",
    confirm: "Confirm",
    ok: "OK",
    back: "← Back",
    delete: "Delete",
    more: "More",
    remove: "Remove",
    dismiss: "dismiss",
    dismissNotice: "Dismiss",
    add: "Add",
    filter: "Filter…",
    noMatch: "No match",
    create: "Create",
    save: "Save",
    loading: "Loading…",
    close: "Close",
  },
  connection: {
    online: "Connected",
    reconnecting: "Reconnecting…",
  },
  nav: {
    toLight: "Switch to light",
    toDark: "Switch to dark",
    search: "Search",
    searchPlaceholder: "Search…",
    settings: "Settings",
    instances: "Instances",
    openInstances: "Open instances",
    closeInstances: "Close instances",
    openFiles: "Open files",
    openTasks: "Open tasks",
    files: "Files",
    tasks: "Tasks",
    closeTasks: "Close tasks",
    hideSidebar: "Hide sidebar",
    showSidebar: "Show sidebar",
    resizePanel: "Drag to resize panel",
    sessions: "Sessions",
    bots: "Bots",
  },
  settings: {
    title: "Settings",
    appearance: "Appearance",
    dark: "Dark",
    light: "Light",
    language: "Language",
    addInstance: "Add an instance",
    instanceNamePlaceholder: "instance name (optional)",
    generateToken: "Generate token",
    runOnHost: "Run on the xacpx host:",
    retentionTitle: "History retention",
    retentionBody: "Keeps the newest {max} messages per session, for up to {days} days. Configured server-side.",
    relayTitle: "Relay",
    relayVersion: "Relay v{version}",
    relayUpdateAvailable: "Update available: v{latest} — run `xacpx-relay update`",
    notifTitle: "Desktop notifications",
    notifOn: "On",
    notifOff: "Off",
    notifUnsupported: "Not supported in this environment",
    notifServerDisabled: "Not enabled on server",
    notifDenied: "Notification permission denied",
    notifDeniedHint: "Allow notifications in the browser site settings, then retry.",
    notifIosHint: "On iPhone and iPad, add XACPX to the Home Screen and open it from there before enabling notifications.",
    notifEnable: "Enable notifications",
    notifDisable: "Disable notifications",
    account: "Account",
    signOut: "Sign out",
    signOutTitle: "Sign out?",
    signOutBody: "You'll need to sign in again to access the dashboard.",
  },
  copy: { copy: "Copy", copied: "Copied" },
  palette: { placeholder: "Jump to a session…", noMatches: "no matches" },
  chat: {
    selectSession: "Select a session",
    loadOlder: "Load earlier messages",
    resizeComposer: "Drag to resize input height",
    browseFiles: "Browse files",
    viewChanges: "View changes",
    changed: "changed",
    tool: "tool",
    tools: "tools",
    resend: "Resend",
    resendThis: "Resend this message",
    failedToSend: "Failed to send",
    stopped: "Stopped",
    failed: "failed",
    truncated: "reply truncated",
    scheduledFor: "Scheduled for",
    scheduled: "Scheduled",
    stop: "Stop",
    send: "Send",
    sentTo: "Sent to",
    fromPeer: "From",
    model: "model",
    modelSetFailed: "Failed to switch model. Try again.",
    effort: "effort",
    effortSetFailed: "Failed to switch effort. Try again.",
    working: "Agent is working… (Esc to stop)",
    // Newline-delimited pool for the turn HUD (never use "|" — vue-i18n reserves it for pluralization).
    workingQuips:
      "Ideas loading…\nSparks flying off the keyboard\nKeyboard smoking — do not disturb\nCPU is sweating a little\nArguing with a bug\nTranslating requirements into code\nLaying it down line by line\nKnee-deep in the docs\nCoffee refilled, back at it\nFingers flying, code flowing\nBrain fans at full speed\nNeurons firing wildly\nPaddling through the knowledge sea\nUnboxing your requirements\nPuzzle in progress — no rushing\nDeleted, rewritten, repeat\nCooking up something big\nGears turning, clickety-clack\nInjecting soul into the code\nBytes flying, bugs fleeing",
    mentionSource: {
      worker: "Worker",
      weixin: "WeChat",
      feishu: "Feishu",
      yuanbao: "Yuanbao",
    },
    mentionActivity: {
      working: "Working",
      waiting: "Waiting",
      idle: "Idle",
    },
    message: "Message",
    mermaidError: "Diagram failed to render",
    mermaidZoomOut: "Zoom out",
    mermaidReset: "Reset",
    mermaidZoomIn: "Zoom in",
    mermaidFullscreen: "Fullscreen",
    mermaidSource: "Show diagram source",
    mermaidDownload: "Download as PNG",
    jumpLatest: "Latest",
    queuedHeader: "Queued",
    queueCancelAria: "Remove from queue",
    contextUsage: "Context: {used} / {size} tokens",
    usage: {
      contextWindow: "Context window",
      tokens: "Tokens (this session)",
      input: "Input",
      output: "Output",
      cacheRead: "Cache read",
      cacheWrite: "Cache write",
      thinking: "Thinking",
      total: "Total",
      cost: "Cost",
      cumulative: "Cumulative",
    },
    attach: { add: "Attach file", remove: "Remove", tooMany: "Up to 5 files per message", tooLarge: "{name} is too large (max 10MB)" },
    lightboxLabel: "Image viewer",
    lightboxOpen: "View fullscreen",
    lightboxPrev: "Previous image",
    lightboxNext: "Next image",
  },
  session: {
    newSession: "New session",
    nativeSession: "Native session",
    existing: "Existing",
    newPath: "New path",
    agent: "Agent",
    workspace: "Workspace",
    selectAgent: "Select an agent",
    selectWorkspace: "Select a workspace",
    selectNative: "Select a native session",
    close: "Close",
    starting: "Starting {agent}…",
    startingHint: "The first start can take 10–40s. This is normal — you can keep browsing while it spins up.",
    startFailed: "Couldn't start the session",
    loadingOptions: "Loading options…",
    source: "Source",
    sessionAlias: "Session alias",
    optional: "(optional)",
    pathPlaceholder: "/abs/path",
    browsePath: "Browse directories on the instance",
    model: "Model",
    toggleModelList: "Toggle model list",
    agentDefault: "agent default",
    listingNative: "Listing native sessions…",
    noNativeSessions: "No native sessions found for this agent + workspace.",
    create: "Create",
    creatingButton: "Creating…",
    configured: "Configured",
    availableDrivers: "Available drivers",
    cliNotDetected: "{driver} — CLI not detected",
    autoAlias: "auto",
    pickConfiguredAgent: "Pick a configured agent to list its native sessions.",
    failedLoadOptions: "failed to load options",
    failedListNative: "failed to list native sessions",
    authHint: "{agent} isn't authenticated on the host — log it in there (e.g. run its CLI login), then retry.",
    thisAgent: "This agent",
    pickNativeAndAlias: "pick a native session and a valid alias",
    couldNotDeriveName: "could not derive a valid name — please enter an alias or a normal path",
    aliasTaken: "A session named “{alias}” already exists — pick a different alias.",
    createFailed: "create failed",
  },
  files: {
    title: "Browse",
    closeFile: "Close file",
    selectToBrowse: "Select a session to browse its workspace",
    searchPlaceholder: "Search files by name…",
    searchContentPlaceholder: "Search file contents…",
    newInRoot: "New file / folder in root",
    find: "Find in file",
    refresh: "Refresh file list",
    upOneLevel: "Up one level",
    changes: "Changes",
    staged: "Staged",
    untracked: "Untracked",
    clearSearch: "Clear search",
    showMore: "Show {n} more",
    showingFirstMatches: "showing first 200 matches",
    root: "root",
    containsChanges: "contains changes",
    emptyDirectory: "empty directory",
    filesCount: "files",
    noChanges: "no changes",
    notGitRepo: "Not a git repository",
    notGitRepoBody: "This workspace has no version control to show changes for.",
    noDiffLoaded: "no diff loaded",
    loading: "loading…",
    truncated: "truncated",
    binary: "binary",
    binaryNotShown: "Binary file not shown.",
    noDiffContent: "No diff content.",
    diffTruncated: "diff truncated",
    detached: "detached",
    linked: "linked",
    worktree: "worktree",
    git: {
      fetch: "Fetch",
      pull: "Pull",
      push: "Push",
      stage: "Stage",
      unstage: "Unstage",
      untrack: "Untrack",
      discard: "Discard changes",
      untrackTitle: "Untrack file?",
      untrackBody: "{path} will be removed from the index but kept on disk. It will reappear as untracked unless ignored.",
      untrackConfirm: "Untrack",
      discardTitle: "Discard changes?",
      discardBody: "Restore {path} to its last committed state. Staged and unstaged changes will be lost.",
      discardConfirm: "Discard",
      discardUntrackedTitle: "Delete untracked file?",
      discardUntrackedBody: "{path} is not tracked by Git. Discarding will permanently delete it from disk.",
      discardUntrackedConfirm: "Delete file",
      stageAll: "Stage all",
      commit: "Commit",
      commitPlaceholder: "Commit message",
      commitStaged: "{n} staged",
      newBranch: "Create branch",
      branchName: "Branch name",
      startPoint: "Start point (optional)",
      create: "Create",
      createBranch: "Create a new branch",
      worktrees: "Worktrees",
      inWorktree: "in worktree",
      newWorktree: "New worktree",
      workspaceName: "Workspace name",
      createAndOpen: "Create & open session",
      noUpstream: "No upstream",
      pushFirstTitle: "Publish branch?",
      pushFirstBody: "Push {branch} to origin and set it as the upstream branch.",
      pushAndTrack: "Push & track",
      errors: {
        dirtyWorktree: "Commit or discard local changes before switching branches or pulling.",
        noUpstream: "This branch has no upstream. Use Push to publish and track it.",
        writeDisabled: "Git changes are disabled. Enable files.writeEnabled on the instance.",
        invalidBranch: "Enter a valid Git branch name.",
        invalidStartPoint: "The selected branch or commit start point does not exist.",
        workspaceExists: "That workspace name already exists. Choose another name.",
        unknownRemote: "The selected Git remote is not configured.",
        detachedHead: "Create or switch to a branch before pushing.",
        inProgress: "Another Git operation is still running.",
      },
      running: {
        stage: "Staging…", unstage: "Unstaging…", untrack: "Untracking…", discard: "Discarding…",
        commit: "Committing…", fetch: "Fetching…",
        pull: "Pulling (fast-forward only)…", push: "Pushing…", checkout: "Switching branch…",
        "worktree-create": "Creating worktree…",
      },
      done: {
        stage: "Staged", unstage: "Unstaged", untrack: "Untracked", discard: "Discarded",
        commit: "Committed", fetch: "Fetch complete",
        pull: "Pull complete", push: "Push complete", checkout: "Branch switched",
        "worktree-create": "Worktree created",
      },
    },
    editFile: "Edit",
    save: "Save",
    saving: "Saving…",
    cancel: "Cancel",
    reload: "Reload",
    unsavedConfirm: "Discard unsaved changes?",
    staleConflict: "This file changed on disk. Reload to get the latest, then re-apply your edits.",
    writeDisabled: "Editing is disabled on this instance.",
    binaryNotEditable: "Binary files can't be edited.",
    tooLarge: "File is too large to save.",
    tree: { expand: "Expand", collapse: "Collapse", emptyFolder: "Empty folder" },
    search: {
      matchCase: "Match case",
      wholeWord: "Match whole word",
      regex: "Use regular expression",
      include: "files to include",
      exclude: "files to exclude",
      byName: "By name",
      byContent: "By content",
      noContentMatches: "No matches in files",
    },
    menu: {
      copyPath: "Copy path",
      copyRelativePath: "Copy relative path",
      searchInFolder: "Search in this folder",
      newFile: "New File",
      newFolder: "New Folder",
      rename: "Rename",
      delete: "Delete",
      download: "Download",
      confirmDelete: 'Delete "{name}"? This cannot be undone.',
      writeDisabled: "File writes are disabled. Enable `files.writeEnabled` in the instance config.",
    },
    toast: {
      created: 'Created "{name}"',
      renamed: 'Renamed to "{name}"',
      deleted: 'Deleted "{name}"',
      downloaded: 'Downloaded "{name}"',
      failed: "Operation failed: {msg}",
    },
    toggle: { showDotfiles: "Show dotfiles", showGitignored: "Show git-ignored" },
  },
  instance: {
    newSession: "New session",
    deleteSession: "Delete session",
    archiveSession: "Sleep",
    sessionArchivedLabel: "sleeping",
    sleepTooltip: "Sleeping closes the session process immediately. History is kept — send a message to wake it.",
    sessionColdTitle: "Process exited — next message cold-starts.",
    sessionTerminalOpenTitle: "Terminal tab is open",
    sessionNativeBadgeTitle: "Resumed from an existing agent-side session",
    sessionArchivedToast: "Slept \"{alias}\" — process closed, message to wake",
    undo: "Undo",
    manage: "Manage instance",
    versionLabel: "Instance version",
    coreVersion: "connector core {version}",
    coreVersionUnknown: "connector core version unknown (legacy or pre-version connector)",
    offline: "offline",
    loading: "loading…",
    noSessions: "no sessions yet",
    loadSessions: "Load sessions",
    loadMoreSessions: "Load more sessions",
    showArchivedSessions: "Show sleeping sessions",
    hideArchivedSessions: "Hide sleeping sessions",
    unarchiveSession: "Wake",
    showMoreSessions: "Show {n} more",
    collapseSessions: "Collapse",
    deleteSessionTitle: "Delete session?",
    deleteSessionBody: "\"{alias}\" will be permanently removed. This can't be undone.",
    manageTitle: "Manage · {name}",
    close: "Close",
    dialogLoading: "Loading…",
    tabGeneral: "General",
    nameLabel: "Name",
    renameSession: "Rename",
    renamePlaceholder: "instance name",
    sessionRenamePlaceholder: "Session name",
    renameSave: "Save",
    renameSaving: "Saving…",
    renameFailed: "rename failed",
    sessionRenameFailed: "Rename failed: {msg}",
    sessionArchiveFailed: "Sleep failed ({alias}): {msg}",
    sessionUnarchiveFailed: "Wake failed ({alias}): {msg}",
    sessionDeleteFailed: "Delete failed ({alias}): {msg}",
    groupModeLabel: "Sidebar grouping",
    groupModeHint: "How this instance's sessions are grouped in the sidebar.",
    groupModeInstance: "None (flat)",
    groupModeWorkspace: "By workspace",
    groupModeAgent: "By agent",
    groupNewSession: "New session in {name}",
  },
  agents: {
    title: "Agents",
    empty: "No agents yet.",
    remove: "Remove {name}",
    removeAria: "Remove agent {name}",
    chooseDriver: "Choose a driver…",
    builtin: "built-in",
    installed: "installed",
    cliNotDetected: "CLI not detected",
    namePlaceholder: "name (optional, = driver)",
    add: "Add agent",
    removeTitle: "Remove agent?",
    removeBody: "\"{name}\" will be removed from this instance.",
    addFailed: "add failed",
    removeFailed: "remove failed",
  },
  workspaces: {
    title: "Workspaces",
    empty: "No workspaces yet.",
    remove: "Remove {name}",
    removeAria: "Remove workspace {name}",
    namePlaceholder: "name",
    pathPlaceholder: "/abs/path",
    browsePath: "Browse directories on the instance",
    descPlaceholder: "description (optional)",
    add: "Add workspace",
    removeTitle: "Remove workspace?",
    removeBody: "\"{name}\" will be removed from this instance.",
    createFailed: "create failed",
    removeFailed: "remove failed",
  },
  dirPicker: {
    title: "Choose directory",
    chooseCurrent: "Choose this directory",
    home: "Home",
    up: "Up one level",
    showHidden: "Show hidden directories",
    empty: "No subdirectories",
    truncated: "Too many directories - list truncated",
    loadFailed: "Failed to load directory",
    pathPlaceholder: "Path (Enter to open)",
  },
  tasks: {
    orchestration: "Orchestration",
    noOrchestration: "No orchestration tasks.",
    cancelTask: "Cancel task",
    cancelOrchestrationAria: "Cancel orchestration task",
    cancelTitle: "Cancel task?",
    cancelBody: "The running orchestration task will be stopped.",
    cancelConfirm: "Cancel task",
    keepRunning: "Keep running",
    scheduled: "Scheduled",
    noScheduled: "No scheduled tasks.",
    viewAll: "View all {count}",
    messagePlaceholder: "message",
    schedule: "Schedule",
    newScheduled: "New scheduled task",
    whenLabel: "When",
    messageLabel: "Message",
    moreOrchestration: "+{count} more",
    allScheduled: "All scheduled tasks",
    scheduledCount: "Scheduled · {count}",
    close: "Close",
    statusUpcoming: "Upcoming",
    statusRunning: "Running",
    statusDone: "Done",
    statusFailed: "Failed",
    statusMissed: "Missed",
    statusCancelled: "Cancelled",
    viewRun: "View run",
    viewRunTitle: "View this run in the conversation",
    cancelScheduled: "Cancel scheduled task",
    cancelScheduledTitle: "Cancel scheduled task?",
    cancelScheduledBody: "It will be removed from the schedule and won't run.",
    keep: "Keep",
  },
  login: {
    window: "login",
    tokenLabel: "Access token",
    tokenPlaceholder: "paste token…",
    runHint: "Run this on the host to mint a token, then paste it below.",
    signIn: "Sign in",
    signingIn: "Signing in…",
    copyCommand: "Copy command",
    toggleVisibility: "Show or hide token",
  },
  invite: {
    window: "invite",
    heading: "You've been invited",
    explainer: "This invite is single-use. Redeeming it creates an account and shows its access token exactly once.",
    redeem: "Redeem invite",
    redeeming: "Redeeming…",
    tokenLabel: "Access token",
    notShownAgain: "Save this token now — it will not be shown again.",
    copyToken: "Copy token",
    signIn: "Sign in with this token",
    connectorHint: "You can also connect an instance with: xacpx channel add relay --token …",
    errorInvalid: "This invite is invalid, expired, or already used.",
    errorRateLimited: "Too many attempts — try again later.",
    errorGeneric: "Something went wrong. Try again.",
    backToLogin: "Back to login",
  },
  fue: { gotIt: "Got it" },
  tools: {
    toolSteps: "Tool steps",
    collapsed: "Tool steps collapsed",
    collapsedBody: "Multi-step tool runs are collapsed by default to keep replies readable. Click the header to expand and inspect each step.",
    subagent: "Subagent",
    running: "Running",
    finished: "Finished",
    failed: "Failed",
    traceCount: "{count} activity steps",
    delegatedTask: "Delegated task",
    report: "Result report",
    updatedAgo: "updated {ago} ago",
    runningNoActivityYet: "Running — no activity details yet…",
    noRecordedActivity: "No recorded activity",
    viewFullTrace: "View full trace",
    loadingDetails: "Loading details…",
    terminalOutputNotReported: "This command ran in the agent's terminal; its output is not reported to the dashboard.",
    kinds: {
      read: "Read",
      search: "Search",
      execute: "Terminal",
      edit: "Edit",
      write: "Write",
      think: "Thinking",
      other: "Tool",
    },
  },
  plan: {
    title: "Plan",
    togglePlan: "Toggle plan",
  },
  reasoning: {
    reasoning: "Reasoning",
    reasoningStreaming: "Reasoning…",
  },
  turnTrace: {
    worked: "Worked",
    tools: "{count} tool step | {count} tool steps",
    files: "{count} file | {count} files",
    failedSteps: "{count} failed | {count} failed",
    thoughts: "{count} thought | {count} thoughts",
    toggleTrace: "Toggle intermediate steps",
    verbs: {
      edited: "edited {count} | edited {count}",
      ran: "ran {count} | ran {count}",
      searched: "searched {count} | searched {count}",
      read: "read {count} | read {count}",
      fetched: "fetched {count} | fetched {count}",
      thought: "planned {count} | planned {count}",
      other: "used {count} other tool | used {count} other tools",
    },
  },
  taskPanel: {
    noSession: "No session selected.",
  },
  center: {
    chat: "Chat",
    terminal: "Terminal",
    desktop: "Desktop",
    closeTab: "Close tab",
  },
  desktop: {
    title: "Desktop",
    unavailable: "Desktop requires an online instance with the desktop capability.",
    disabled: "Desktop is disabled. Enable `desktop.enabled` in the instance config.",
    offline: "Instance is offline.",
    busy: "Another desktop viewer is active on this instance.",
    rfbUnavailable: "No VNC server is listening on the instance loopback port.",
    notRfb: "The desktop port is not speaking RFB/VNC.",
    authUnsupported: "This VNC auth scheme is not supported (None, VeNCrypt, and ARD need Phase B).",
    authFailed: "The VNC server rejected the password. Check it and reconnect.",
    streamTimeout: "Desktop open timed out. Retry in a moment.",
    statusIdle: "Idle.",
    statusOpening: "Requesting desktop…",
    statusAuth: "VNC password required.",
    statusConnecting: "Connecting to desktop…",
    statusOpen: "Connected.",
    statusClosed: "Disconnected.",
    statusError: "Desktop error.",
    errorTitle: "Could not open the desktop.",
    passwordPlaceholder: "VNC password",
    connect: "Connect",
    disconnect: "Disconnect desktop",
    reconnect: "Reconnect",
    fit: "Fit to window",
    actual: "Actual size",
    fullscreen: "Fullscreen",
    exitFullscreen: "Exit fullscreen",
  },
  terminal: {
    title: "Terminal",
    unavailable: "Terminal requires an online instance with RMUX capabilities.",
    disabled: "Terminal is disabled. Enable `terminal.enabled` in the instance config.",
    unsupported: "Terminal is not supported on this instance platform.",
    offline: "Instance is offline.",
    noSession: "Select a session to open a terminal.",
    exited: "Terminal exited ({code}).",
    error: "Could not open the terminal.",
    eventsOffline: "Dashboard connection lost. Reconnecting…",
    reconnecting: "Instance is reconnecting. Retry in a moment.",
    sessionNotFound: "Session was not found.",
    sessionArchived: "Session is sleeping; wake it before opening a terminal.",
    capacityExceeded: "Terminal capacity exceeded on this instance.",
    viewerCapacityExceeded: "Too many viewers on this terminal.",
    closeConfirm: "Close this terminal? The shared shell will be terminated.",
    closeSharedConfirm: "This terminal is open on other devices. Closing terminates it everywhere. Continue?",
    closeRetryToast: "Could not close the terminal (offline or timed out). Keep the tab and retry.",
    closeError: "Could not close the terminal.",
    cleanupPendingToast: "Terminal cleanup is still pending on the instance.",
    takeControl: "Take control",
    spectatorHint: "Read-only. Take control to type.",
    viewers: "{count} viewers",
    role: {
      controller: "Controller",
      spectator: "Spectator",
    },
    keybar: {
      show: "Show shortcut bar",
      hide: "Hide shortcut bar",
      paste: "Paste",
      copy: "Copy",
      up: "Up",
      down: "Down",
      left: "Left",
      right: "Right",
    },
  },
  agentMessage: {
    waitingForCompletion: "Waiting for completion",
    waitingForResult: "Waiting for result",
    resultReturned: "Result ready",
    completed: "Peer completed",
    failed: "Failed",
    cancelled: "Cancelled",
    sent: "Sent",
    delivered: "Delivered",
    sending: "Sending",
  },
  notifications: {
    taskCompleted: "Task completed",
    taskFailed: "Task failed: {error}",
    unknownError: "Unknown error",
  },
  bot: {
    nav: {
      title: "Direct Bot",
    },
    list: {
      empty: "No bots on this instance.",
    },
    status: {
      enabled: "Enabled",
      disabled: "Disabled",
    },
    actions: {
      newBot: "New Bot",
      edit: "Edit Bot",
      delete: "Delete Bot",
    },
    delete: {
      confirmTitle: "Delete Bot",
      confirmMessage: "Are you sure you want to delete \"{name}\"? This cannot be undone.",
      failedTitle: "Failed to delete Bot",
    },
    dialog: {
      createTitle: "New Bot",
      editTitle: "Edit Bot",
    },
    fields: {
      name: "Name",
      namePlaceholder: "e.g. Code Reviewer",
      role: "Role / Description",
      rolePlaceholder: "e.g. Senior code reviewer",
      agent: "Agent",
      workspace: "Workspace",
      instructions: "Instructions",
      instructionsPlaceholder: "System prompt instructions for this bot...",
      instructionsHint: "Changes apply to future turns and do not erase existing conversation history.",
      model: "Model (optional)",
      modelPlaceholder: "Default or specific model",
      effort: "Reasoning Effort (optional)",
      effortDefault: "Default",
      avatar: "Avatar (optional)",
      avatarPlaceholder: "URL or icon identifier",
      enabled: "Enabled",
    },
    validation: {
      nameRequired: "Bot name is required.",
      agentRequired: "Agent is required.",
      workspaceRequired: "Workspace is required.",
      detailLoading: "Bot details are still loading. Wait a moment before saving.",
      detailLoadFailed: "Could not load Bot details ({msg}). Retry before saving.",
    },
    lifecycle: {
      identityLocked: "Agent and workspace are locked once this Bot has run. Other settings still apply to future turns.",
      deleteBlocked: "This Bot has conversation history and cannot be deleted. Create a new Bot for a fresh identity; history is retained by design.",
    },
    topic: {
      label: "Topic",
      default: "Default",
      new: "New Topic",
      createTitle: "Create New Topic",
      createHint: "A new topic keeps the bot settings but starts with fresh conversation context.",
      titlePlaceholder: "e.g. Refactor API",
    },
    prompt: {
      placeholder: "Type a message... (Enter to send, Shift+Enter for newline)",
      send: "Send",
      stop: "Stop",
      stopRun: "Stop Run",
      retry: "Retry",
      botDisabledWarning: "This bot is disabled. Enable it in Bot Settings to send messages.",
      botDisabledPlaceholder: "Bot is disabled",
      runActivePlaceholder: "Run in progress... (Stop to cancel)",
      recoveringPlaceholder: "Recovering topic... (loading history)",
    },
    chat: {
      emptyHint: "Send a message to start a conversation with this bot.",
      working: "Working",
    },
    run: {
      failed: "Run failed",
      cancelled: "Run cancelled",
      indeterminate: "Indeterminate",
      indeterminateHint: "The run outcome is uncertain due to cancellation/timeout. State will reconcile when available.",
      unknownError: "An unexpected error occurred.",
      queued: "Queued",
      running: "Running",
      waitingHuman: "Waiting for human",
    },
    errors: {
      connectorOutdated: "This feature needs a newer connector — rebuild and reconnect the relay channel on that instance.",
      discoveryFailed: "Run discovery failed. History loaded, but the live Run owner is unknown — retry to confirm before sending.",
      ownershipUnconfirmed: "Ownership unconfirmed due to discovery failure. Retry discovery to confirm active run.",
      ownershipChecking: "Ownership unconfirmed. Checking active run...",
      topicRecovering: "Topic is still recovering. Wait for history to finish loading before sending.",
      botDisabled: "Bot is disabled. Enable it before sending messages.",
      runInProgress: "A run is already in progress. Wait for it to finish or cancel it.",
      cancelUnknown: "Cancellation outcome unknown. Waiting for instance state...",
      instanceOffline: "Instance is offline",
    },
  },
} as const;
