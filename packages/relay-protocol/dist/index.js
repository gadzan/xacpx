...
function validConversationRun(value) {
  if (typeof value !== "object" || value === null)
    return false;
  const c = value;
  return typeof c.id === "string" && typeof c.conversationId === "string" && typeof c.topicId === "string" && typeof c.requestMessageId === "string" && typeof c.requestId === "string" && (c.mode === "explicit" || c.mode === "automatic") && (c.state === "queued" || c.state === "running" || c.state === "waiting-human" || c.state === "completed" || c.state === "failed" || c.state === "cancelled" || c.state === "indeterminate") && typeof c.profileRevision === "number" && typeof c.maxMemberTurns === "number" && typeof c.consumedMemberTurns === "number" && Array.isArray(c.failedBotIds) && c.failedBotIds.every((entry) => typeof entry === "string") && Array.isArray(c.unavailableBotIds) && c.unavailableBotIds.every((entry) => typeof entry === "string") && typeof c.createdAt === "string" && optStr(c.completionReason) && optStr(c.startedAt) && optStr(c.finishedAt) && (c.activeBatch === undefined || typeof c.activeBatch === "number");
}
...
var NOTICE_KINDS = new Set(["task-completion", "task-progress", "coordinator-message", "queue-overflow"]);
