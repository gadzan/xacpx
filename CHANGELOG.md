...
- Runtime worker EOF discharge (#363): `"verified"` is a per-ATTEMPT statement, not a property of the accumulated evidence. `mergeEvidence` OR-s the two attempts' flags, so a later snapshot that saw nothing could retire a descendant an earlier attempt had captured — the descendants worker kills root-first, so the `ParentProcessId` edge that discovered a late process stops existing and Windows gives no guarantee it is ever re-observed. That descendant was dropped before publication ran, leaving `"spooled"` unreachable and no residual written while the worker claimed a clean tree. Discharge now reads the attempt's own flag AND the merged set of unresolved identities (non-safe outcomes plus leftovers), and `mergeEvidence`'s `verified` field is documented as unusable for a terminal decision. A clean attempt holding no unresolved identity still returns `"verified"` in one round.
...
- Managed adapter pins refreshed (ACP initialize probe passes on both): Codex `1.1.9` → `1.10.0`, Claude `0.64.2` → `0.75.1`. Recorded generated adapter commands refresh to the new defaults automatically; custom recorded commands stay sticky. Preinstalled adapters are version-pinned too: an active preinstall of the old versions no longer matches and falls back to the new `npx` path (needs network on cold start), so re-run `xacpx adapter preinstall <codex|claude>` after upgrading to keep offline startup. Queue-owner reaping now covers previous-pin identities: a crash + pin upgrade no longer orphans the old owner (startup/shutdown/periodic reap emits both the current and the last-dispatched launch identity; `ttl=0` owners included). Worker binding rebuilds preserve the launch snapshot, and worker retirement converges the snapshotted identity before deleting the binding, so the evidence survives reuse and terminal-ephemeral reconciliation.
