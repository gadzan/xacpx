...
- Feishu channel: opt-in `trustGroupOwner` option - asserts `isOwner` on group turns whose sender is the chat owner (GET `/im/v1/chats`, cached 5 min, fail closed on lookup errors with a 30 s failure sentinel; lifecycle-epoch guarded so lookups that straddle a logout never repopulate the cache), so owner-gated control commands work without configuring `ownerIds`. Off by default; only enable when you control who can add the bot to groups.
