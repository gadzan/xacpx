...
test("managed adapters use tested exact defaults and accept local overrides", () => {
  expect(effectiveAdapterVersion("codex", {})).toBe("1.12.0");
  expect(effectiveAdapterVersion("claude", {})).toBe("0.78.0");
...
test("recognizes only generated commands for managed adapter packages", () => {
  ...
  expect(isManagedAdapterCommand("codex", "npx -y @agentclientprotocol/codex-acp@1.12.0")).toBe(true);
  ...
  expect(isManagedAdapterCommand("claude", "npx -y @agentclientprotocol/codex-acp@1.12.0")).toBe(false);
});
