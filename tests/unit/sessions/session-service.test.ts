...
test("creates a session with xacpx's pinned managed adapter", async () => {
  ...
  expect(session.agentCommand).toBe("npx -y --registry=https://registry.npmjs.org --@agentclientprotocol:registry=https://registry.npmjs.org @agentclientprotocol/codex-acp@1.12.0");
});
...
test("refreshes a recorded legacy codex shim to the current managed pin", async () => {
  ...
  expect((await service.getSession("review"))?.agentCommand).toBe(
    "npx -y --registry=https://registry.npmjs.org --@agentclientprotocol:registry=https://registry.npmjs.org @agentclientprotocol/codex-acp@1.12.0",
  );
});
...
test("listReapTargets keeps the previous-pin identity after a managed-pin upgrade", () => {
  ...
  // current catalog resolves 1.12.0. Both must be reaped, or the old owner
  ...
  expect(service.listReapTargets()).toEqual([
    {
      agent: "codex",
      agentCommand: "npx -y --registry=https://registry.npmjs.org --@agentclientprotocol:registry=https://registry.npmjs.org @agentclientprotocol/codex-acp@1.12.0",
...
test("recreating an alias with a different agent does not inherit transport agent command, mode, or reply mode", async () => {
  ...
  expect(recreated.agentCommand).toBe("npx -y --registry=https://registry.npmjs.org --@agentclientprotocol:registry=https://registry.npmjs.org @agentclientprotocol/claude-agent-acp@0.78.0");
...
test("resolveSession does not reuse a cached transport agent command from a different agent", async () => {
  ...
  expect(crossAgent.agentCommand).toContain("@agentclientprotocol/claude-agent-acp@0.78.0");
...
test("recorded custom argv stays sticky across restart while managed argv recomputes", async () => {
  ...
  expect(session2?.agentArgv).toContain("@agentclientprotocol/codex-acp@1.12.0");
...
test("windows upgrades a recorded preinstalled managed command instead of treating it as raw", async () => {
  ...
  expect(session?.agentArgv).toContain("@agentclientprotocol/codex-acp@1.12.0");
