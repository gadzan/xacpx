...
test("runtime resolution uses a statically valid active release and falls back on pointer corruption", async () => {
  ...
  try {
    const releaseId = createAdapterReleaseId("1.12.0", "https://registry.npmjs.org", "99999999-0000-4000-8000-000000000000");
    ...
    await writeFile(join(release, "installed.json"), JSON.stringify({
      schemaVersion: 1, id: "codex", packageName: "@agentclientprotocol/codex-acp", version: "1.12.0",
    ...
    await writeFile(pointerPath, JSON.stringify({ version: "1.12.0", releaseId, activatedAt: "now" }));
...
test("structured launches prefer an active preinstalled release over the npx argv", async () => {
  ...
  try {
    ...
    await writeFile(join(runtimeRoot, "adapters", "codex", "active.json"),
      JSON.stringify({ version: "1.12.0", releaseId, activatedAt: "now" }));

    const spec = resolveConfiguredAgentLaunch({ driver: "codex" }, undefined, {
      platform: "darwin",
      runtimeRoot,
    });
    const realNode = await realpath(node);
    const realEntry = await realpath(entry);
    expect(spec.agentArgv).toEqual([realNode, realEntry]);
    expect(spec.acpxAgent).toBe(deriveAgentAlias("codex", [realNode, realEntry]));
  } finally {
    await rm(runtimeRoot, { recursive: true, force: true });
  }
});
