...
function make(enabled: boolean, calls: string[]) {
  ...
  const deps: any = {
    workspaces: {
      list: () => [{ name: "ws", cwd: "/nonexistent-xacpx-test" }],
    },
...
test("fsCreate is rejected with files-write-disabled when the gate is off", async () => {
  ...
  await expect(svc.fsCreate("ws", "x.txt", "file")).rejects.toThrow(
    "files-write-disabled",
  );
});
test("fsDelete/fsRename/fsCopy are gated too", async () => {
  const svc = make(false, []);
  await expect(svc.fsDelete("ws", "x.txt")).rejects.toThrow(
    "files-write-disabled",
  );
  await expect(svc.fsRename("ws", "x.txt", "y.txt")).rejects.toThrow(
    "files-write-disabled",
  );
  await expect(svc.fsCopy("ws", "x.txt")).rejects.toThrow(
    "files-write-disabled",
  );
});
test("fsDownload is NOT gated (reaches WorkspaceFs, fails on missing workspace root)", async () => {
  ...
  await expect(svc.fsDownload("ws", "x.txt")).rejects.not.toThrow(
    "files-write-disabled",
  );
});
