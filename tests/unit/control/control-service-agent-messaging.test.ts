...
function makeControlService(agentMessagingDeps?: unknown) {
  ...
  const service = new ControlService({
    ...
    sessions: {
      ...
      removeSession: async () => {},
      useSession: async () => ({ alias: "main", agent: "codex", workspace: "ws", transportSession: "ts", created_at: "", last_used_at: "" }),
    ...
    scheduled: { listPending: () => [], listRecentForChat: () => [], createTask: async () => ({} as never), cancelPending: async () => false },
    orchestration: { listTasks: async () => [], getTask: async () => null, requestTaskCancellation: async () => ({} as never) },
    events,
    agents: { list: () => [], catalog: () => [], create: async () => ({} as never), remove: async () => {} },
    workspaces: { list: () => [], create: async () => ({} as never), remove: async () => {} },
