/**
 * Windows legacy raw `--agent` selector vs acpx@0.16.0 (spike written against 0.13.0; backfill verified still present on 0.15.1 and 0.16.0).
 *
...
test("sanity: installed acpx is 0.16.x (spike depends on built-in backfill)", async () => {
  ...
  expect(pkg.version.startsWith("0.16.")).toBe(true);
