...
async function latestRecord(
  h: Harness,
): Promise<{ recordId: string; record: Record<string, unknown> }> {
  const files = (await readdir(h.sessionsDir)).filter(
  ...
  return {
    recordId,
    record: JSON.parse(
      await readFile(join(h.sessionsDir, name), "utf8"),
...
test(
  "acpx-cli lifecycle: ensure → prompt → follow-up → list → cancel → close, with exact argv boundaries",
  async () => {
...
test(
  "acpx-bridge lifecycle: ensure → prompt → follow-up → list → cancel → close, with exact argv boundaries",
  async () => {
    const h = await makeHarness();
    try {
...
test(
  "legacy record without agent_argv is backfilled and resumed with the same record id",
  async () => {
    const h = await makeHarness();
    ...
    try {
      ...
      await writeFile(
        join(h.sessionsDir, `${legacyRecordId}.json`),
...
test(
  "mcpCoordinatorSession spawns a warm queue owner that the second turn reuses, and the reaper frees it without closing the session",
  async () => {
    const h = await makeHarness();
    const transport = await makeCliTransport(h);
    try {
      const spec = session("warm-demo", BOUNDARY_ARGV, h, {
        mcpCoordinatorSession: "coord-1",
      });
      await writeOverlay(h, spec.acpxAgent!, BOUNDARY_ARGV);
      ...
      const reaped = await reapQueueOwners(
        ACPX,
        [
          {
            agent: spec.agent,
      ...
      if (process.platform === "win32") {
        ...
        try {
          process.kill(ownerBeforeReap!, 0);
          ownerStillAlive = true;
        } catch {
          /* gone */
        }
        console.log(
          "[compat] after reap: pid=%s alive=%s",
          ownerBeforeReap,
          ownerStillAlive,
        );
...
test("migration rejects an identity mismatch and preserves the record", async () => {
  ...
  try {
    ...
    await mkdir(h.sessionsDir, { recursive: true });
    await writeFile(
      join(h.sessionsDir, `${recordId}.json`),
      `${JSON.stringify(
        {
          schema: "acpx.session.v1",
    ...
    const result = await migrateSessionArgvFile(
      recordId,
      {
...
test(
  "acpx-cli message injection: injectMessage queues next-turn prompt on real acpx queue owner",
  async () => {
    const h = await makeHarness();
    const transport = await makeCliTransport(h);
    try {
      ...
      const first = await transport.prompt(spec, "turn-1");
      expect(first.text).toContain("reply=turn-1");

      const receipt = await transport.injectMessage!(spec, {
        text: '<xacpx-message id="msg_cli_1">injected-peer-text</xacpx-message>',
        messageId: "msg_cli_1",
        mode: "auto",
      });
...
test(
  "acpx-bridge message injection: injectMessage queues next-turn prompt on real acpx queue owner",
  async () => {
    const h = await makeHarness();
    ...
    try {
      ...
      const receipt = await transport.injectMessage!(spec, {
        text: '<xacpx-message id="msg_bridge_1">injected-bridge-text</xacpx-message>',
        messageId: "msg_bridge_1",
        mode: "auto",
      });
      expect(receipt.status).toBe("queued");
      expect(receipt.modeUsed).toBe("queue");
