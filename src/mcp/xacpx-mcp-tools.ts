      ...
      inputSchema: z
        .object({
          ...
          mode: z
            ...
            .optional()
            .describe(
              "Delivery mode (default: auto -> queue). 'interrupt' CANCELS the target's current turn and delivers this message as its next turn — only when waiting would materially harm the task. 'auto' and 'queue' never cancel. 'steer' injects into the target's active turn when the runtime supports it.",
            ),
          replyTo: z
            .string()
            .min(1)
